...
def remove_fields_if_exist(apps, schema_editor):
    """Elimina campos de Alumno solo si existen (idempotente)"""
...
def add_fields_if_not_exist(apps, schema_editor):
    """Agrega campos a Configuracion solo si no existen (idempotente)"""
    from django.db import connection
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT column_name
        ...
        fields_to_add = [
            ('aspirantes_forzar_carga_completa', "BOOLEAN DEFAULT FALSE"),
            ...
            ('email_usar_microsoft_graph', "BOOLEAN DEFAULT FALSE"),
            ('ingresantes_forzar_carga_completa', "BOOLEAN DEFAULT FALSE"),
            ('preinscriptos_forzar_carga_completa', "BOOLEAN DEFAULT FALSE"),
        ]
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('alumnos', '0031_remove_alumno_email_payload_and_more'),
    ]
